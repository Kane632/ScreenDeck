...
StartUnrealInsight()
{
	...
	If (WinExist(GGeneratedConfig["InsightWindowName"]))
    {
		WinActivate(GGeneratedConfig["InsightWindowName"])
	}
	Else
	{
		Run(GGeneratedConfig["InsightPath"])
	}
}
...
{
	global GGeneratedConfig
	If (WinExist(GGeneratedConfig["FrontendWindowName"]))
    {
		WinActivate(GGeneratedConfig["FrontendWindowName"])
    }
	Else
	{
		Run(GGeneratedConfig["FrontendPath"])
	}
}
...
StartIDEBackend()
{
	; global ProjectBackendSlnPath, RiderBackendWindowName, RiderPath
	; If (!WinExist(RiderBackendWindowName))
    ; {
	; 	RunStr := RiderPath . "\rider64.exe " . ProjectBackendSlnPath
    ;     Run(RunStr)
    ; }
	; Else
	; {
	; 	WinActivate(RiderBackendWindowName)
	; }
}
...
{
	global GGeneratedConfig
	If (WinExist(GGeneratedConfig["VisualStudioWindowName"]))
    {
		WinActivate(GGeneratedConfig["VisualStudioWindowName"])
	}
	Else
	{
		Run(GGeneratedConfig["ProjectSlnPath"])
	}
}

StartRider()
{
	global GGeneratedConfig, GBaseConfig

	If (WinExist(GGeneratedConfig["RiderWindowName"]))
    {
		WinActivate(GGeneratedConfig["RiderWindowName"])
    }
	Else
	{
		RunStr := GBaseConfig["RiderPath"] . "\rider64.exe " . GGeneratedConfig["ProjectSlnPath"]
		Run(RunStr)
	}
}
...
{
	global GGeneratedConfig
	If (WinExist(GGeneratedConfig["UprojectWindowName"]))
    {
		WinActivate(GGeneratedConfig["UprojectWindowName"])
	}
	Else
	{
		RunStr := GGeneratedConfig["UnrealSourceExe"] . " " . GGeneratedConfig["ProjectUprojectPath"]
		Run(RunStr)
	}
}
...
StartGitkraken()
{
	...
	If (WinExist(GGeneratedConfig["GitkrakenWindowName"]))
    {
		WinActivate(GGeneratedConfig["GitkrakenWindowName"])
	}
	Else
	{
		;Paths
		AppDataLocal 		:= SubStr(A_AppData,1,StrLen(A_AppData)-7) . "Local"
		GitKrakenFolder 	:= AppDataLocal . "\gitkraken"
		GitKrakenExe 		:= GitKrakenFolder . "\Update.exe"
	
		RunCommand := gitKrakenExe . " --processStart `"gitkraken.exe`" --process-start-args=`"--path \`"" . GBaseConfig["ProjectPath"] . "`""
		Run(A_ComSpec " /c " RunCommand)
	}
}
...
StartPlastic()
{
	...
	If (WinExist(GGeneratedConfig["PlasticWindowName"]))
	{
		WinActivate(GGeneratedConfig["PlasticWindowName"])
	}
	Else
	{
		RunCommand := '"' . GBaseConfig["PlasticPath"] . '\plastic.exe"'
		Run(RunCommand)
	}
}

StartCurrentLaunchConfig()
{
	global Logger
	Logger.Log("StartCurrentLaunchConfig.")
}